...
const App: React.FC = () => {
  const [viewMode, setViewMode] = useState<string>('buy');
  ...
  useEffect(() => {
    loadData();
    // Jei nėra DB raktų, po 1.5 sek priverstinai atidaryti nustatymus
  ...
  const filteredParts = useMemo(() => {
    return parts.filter(part => {
      ...
      return part.title.toLowerCase().includes(term) || part.brand.toLowerCase().includes(term);
    });
  }, [parts, searchTerm]);
      ...
      <main className="flex-1 max-w-7xl mx-auto px-6 py-16 w-full">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center py-40">
            ...
            <p className="text-[10px] font-black uppercase text-slate-400 tracking-widest">Kraunami garažo lobiai...</p>
          </div>
        ) : (
          <>
            <div className="mb-10 flex justify-between items-end">
              <div>
                <h2 className="text-4xl font-black text-slate-900 uppercase italic tracking-tighter">Lobiai tavo projektui</h2>
                <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest mt-2">Iš žmonių garažų - į tavo garažą</p>
              </div>
              <button 
                onClick={() => setShowAI(true)}
                className="bg-slate-900 text-white p-4 rounded-2xl hover:bg-orange-600 transition-all shadow-xl flex items-center gap-2 group"
              >
