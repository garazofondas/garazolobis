...
const App: React.FC = () => {
  ...
  const handleAddPart = async (newPart: Part) => {
    ...
    try {
      ...
      await loadData(); // Persikrauname iš DB
      setIsUploadModalOpen(false);
      setViewMode('sell');
    } catch (e) {
      alert("Klaida saugant DB. Patikrink Supabase nustatymus.");
    } finally {
      setIsLoading(false);
    }
  };
    ...
    if (isLoading) {
      return (
        <div className="flex flex-col items-center justify-center py-32">
          ...
          <p className="font-black text-xs uppercase text-slate-400 tracking-widest">Sujungiam garažus...</p>
        </div>
      );
    }
        ...
        return (
          <div className="max-w-4xl mx-auto space-y-4 animate-in fade-in slide-in-from-bottom-4">
            ...
            {chats.length === 0 ? (
              ...
              chats.map(chat => (
                <div key={chat.id} onClick={() => setActiveChat(chat)} className="bg-white p-4 rounded-2xl border border-slate-200 flex items-center gap-4 cursor-pointer hover:border-orange-500 transition-all shadow-sm">
        ...
        return (
          <div className="max-w-3xl mx-auto animate-in fade-in slide-in-from-bottom-4">
             <div className="bg-slate-900 rounded-[3rem] p-12 text-white shadow-2xl relative overflow-hidden border border-white/10">
                <div className="absolute -top-10 -right-10 w-48 h-48 bg-orange-600/30 rounded-full blur-[80px]"></div>
                <h4 className="text-[10px] font-black uppercase text-slate-400 tracking-widest mb-2">Tavo balansas</h4>
                <h2 className="text-7xl font-black mb-12 tracking-tighter italic">€{walletBalance.toFixed(2)}</h2>
                <div className="flex flex-col sm:flex-row gap-4">
                  <button 
                    disabled={walletBalance <= 0}
                    onClick={() => setShowWithdraw(true)} 
                    className={`px-10 py-5 rounded-2xl font-black uppercase text-xs tracking-widest shadow-xl transition-all ${walletBalance > 0 ? 'bg-orange-600 hover:bg-orange-700 hover:-translate-y-1 shadow-orange-900/40' : 'bg-slate-700 text-slate-500 cursor-not-allowed'}`}
                  >
                    Išsigryninti per Stripe
                  </button>
                </div>
             </div>
          </div>
        ...
        return (
          <div className="max-w-5xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-8">
            <div>
               ...
               <p className="text-[10px] text-slate-400 font-black uppercase tracking-[0.3em] mt-3">Tavo detalių kelionė DPD tinklu</p>
            </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-3 mt-4">
                   ...
                   <p className="text-[10px] text-slate-400 font-black uppercase tracking-widest">
                    {filteredParts.length} rastų elementų realiu laiku
                  </p>
                </div>
              </div>
            </div>
  ...
  return (
    <div className="min-h-screen flex flex-col bg-slate-50">
      ...
      <button 
        ...
        className="fixed bottom-10 right-10 z-[190] w-20 h-20 bg-slate-900 text-white rounded-[2.5rem] shadow-2xl flex items-center justify-center hover:bg-orange-600 transition-all hover:scale-110 active:scale-95 group border-4 border-white"
      >
        <svg className="w-10 h-10 group-hover:rotate-12 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2-2v10a2 2 0 002 2z" /></svg>
        <span className="absolute -top-3 -right-3 bg-red-600 text-white text-[9px] font-black px-3 py-1.5 rounded-full animate-bounce shadow-lg">LIVE AI</span>
      </button>
