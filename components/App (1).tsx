        ...
        return (
          <div className="max-w-4xl mx-auto space-y-6 animate-in fade-in slide-in-from-bottom-4">
            ...
            {myOrders.length === 0 ? (
              ...
              myOrders.map(order => (
                <div key={order.id} className="bg-white p-6 rounded-2xl border border-slate-100 flex flex-col md:flex-row md:items-center gap-6 shadow-sm hover:border-orange-200 transition-all overflow-hidden relative">
                   <div className={`absolute top-0 left-0 w-1 h-full ${order.status === OrderStatus.PENDING_SHIPMENT ? 'bg-amber-400' : 'bg-blue-400'}`}></div>
                  <img src={order.part.imageUrl} className="w-20 h-20 md:w-24 md:h-24 rounded-xl object-cover m-4" />
                  <div className="flex-1 px-4 md:px-0 py-4">
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <h4 className="font-black text-slate-900 uppercase italic leading-none">{order.part.title}</h4>
                        <p className="text-[9px] text-slate-400 uppercase font-black tracking-widest mt-1">Siuntos numeris: <span className="text-indigo-600">{order.trackingNumber}</span></p>
                      </div>
                      <span className={`text-[8px] font-black px-2 py-1 rounded-lg uppercase tracking-widest ${getStatusColor(order.status)}`}>{order.status}</span>
                    </div>
                    
                    {/* Progress Bar Tracking */}
                    <div className="mt-4 flex items-center gap-2 mb-4">
                      {[OrderStatus.PENDING_SHIPMENT, OrderStatus.IN_TRANSIT, OrderStatus.READY_FOR_PICKUP, OrderStatus.DELIVERED].map((s, idx) => {
                         const statuses = [OrderStatus.PENDING_SHIPMENT, OrderStatus.IN_TRANSIT, OrderStatus.READY_FOR_PICKUP, OrderStatus.DELIVERED];
                         const currentIdx = statuses.indexOf(order.status);
                         const isPast = idx <= currentIdx;
                         return (
                           <React.Fragment key={s}>
                             <div className={`w-3 h-3 rounded-full flex-shrink-0 border-2 ${isPast ? 'bg-orange-600 border-orange-600' : 'bg-white border-slate-200'}`}></div>
                             {idx < 3 && <div className={`h-0.5 flex-1 ${isPast ? 'bg-orange-600' : 'bg-slate-200'}`}></div>}
                           </React.Fragment>
                         );
                      })}
                    </div>
                    ...
                    <div className="flex items-center gap-4 text-[10px] text-slate-500 font-bold uppercase">
                      <div className="flex items-center gap-1"><span className="text-slate-400">Siuntimas:</span> {order.locker.type}</div>
                      <div className="flex items-center gap-1"><span className="text-slate-400">Vieta:</span> {order.locker.city}</div>
                    </div>
                  </div>
                  <div className="p-4 md:pr-8 flex md:flex-col gap-2 justify-end">
                    <button onClick={() => setActiveOrder(order)} className="p-3 bg-slate-900 text-white hover:bg-orange-600 rounded-xl transition-all shadow-lg" title="Siuntos informacija">
                      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                    </button>
                    <button className="p-3 bg-white border border-slate-200 text-slate-400 hover:text-indigo-600 rounded-xl transition-all" title="Susisiekti su pardavėju">
