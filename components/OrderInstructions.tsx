...
const OrderInstructions: React.FC<OrderInstructionsProps> = ({ order, onClose, onUpdate }) => {
  ...
  const handleMarkAsShipped = async () => {
    ...
    try {
      ...
      alert("Valio! Pirkėjui pranešta, kad detalė jau keliauja.");
      onClose();
    } catch (e) {
    ...
    <div className="fixed inset-0 z-[110] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
      <div className="bg-white rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in duration-200">
        <div className="p-6 text-center">
          ...
          <p className="text-slate-500 text-sm mb-6 font-medium">Paruošk siuntą išsiuntimui.</p>
          
          <div className="bg-slate-50 border-2 border-dashed border-slate-200 rounded-2xl p-6 mb-6">
            <div className="text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1">{order.locker.type} durelių kodas</div>
          ...
          <div className="space-y-3 mb-8">
            {labelUrl ? (
              <a 
                ...
                className="w-full flex items-center justify-center gap-3 py-4 bg-indigo-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-indigo-700 transition-all shadow-lg"
              >
            ...
            {order.status === OrderStatus.PENDING_SHIPMENT && (
              <button 
                ...
                disabled={isUpdating}
                className="w-full py-4 bg-emerald-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-emerald-700 transition-all shadow-lg flex items-center justify-center gap-3"
              >
                {isUpdating ? <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div> : 'Įdėjau į paštomatą'}
              </button>
            )}
          </div>
